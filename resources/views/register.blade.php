...
<div class="flex items-center justify-center min-h-screen"
    style="background: linear-gradient(to right, #736fdc, #ebcbeb);">
    <div class="bg-white rounded-3xl  shadow-lg h-[495px] flex w-[800px] relative">
        ...
        <div class="w-1/2 py-5 pl-16">
            ...
            <form action="{{ route('register.post') }}" method= "POST" class="mt-5 space-y-2 ">
                @csrf
                <!-- Input Nama Lengkap -->
                <div>
                    <label for="email" class="text-xs font-medium text-black ">Email</label>
                    <input type="text" id="email" name="email"
                        class="w-full px-4 py-2 text-xs border rounded-2xl border-[#8880DF] focus:outline-none focus:ring-1 "
                ...
                <!-- Tombol Submit -->
                <div class="pt-8">
                    <button type="submit" class="w-full py-2 text-white  rounded-full bg-[#8880DF] ">Sign Up</button>
                    <!-- ke arah home-->
                </div>
            </form>
        </div>
